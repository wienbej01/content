...
| Sprint 5 | Contract media QA | `valid` means contract-valid and sync-valid, not only file-valid |
| Sprint 6 | Repair loop | Repair acts on validation failures and preserves audit history |
| Sprint 7 | Assembly preflight | Assembly consumes valid artifacts and enforces timeline heuristics |
...
Assembly must consume only active, validated DB artifacts, and enforce safe timeline pacing to prevent looping errors.
...
7. Media QA validates render-method contracts and lipsync continuity boundaries.
8. Repair acts on validation failures and preserves history.
9. Assembly consumes only active validated DB artifacts and enforces timeline pacing limits.
