# Kilo CLI Prompt — LIVE Smoke Loop with Subagents

Paste this entire prompt into Kilo CLI to start the loop:

---

Execute the LIVE end-to-end smoke test loop defined in `docs/plans/e2e_smoke_loop/PROCEDURE.md`. This is a LIVE test — NO mocks, NO YT_TEST_MODE, NO HIGGSFIELD_DRY_RUN. Real ElevenLabs TTS, real Higgsfield Seedance/Kling video generation, real FFmpeg assembly.

## Your Role

You are the orchestrator. For each of the 25 seeds (listed in `docs/plans/e2e_smoke_loop/SEEDS.md`), you will use the Task tool to spawn 3 subagents in sequence:
...
For each seed (1 through 25), do the following:
...
```
You are the Engineer in a smoke test loop. Your job:
...
```
You are the Auditor in a smoke test loop. Review the Engineer's output for seed #<N>.
...
```
You are the Validator in a smoke test loop. This is the final authority for seed #<N>.
...
If ALL 4 gates pass: print "SEED #<N>: GREEN"
If ANY gate fails: print "SEED #<N>: RED" with the specific failure

Only GREEN seeds count. Do NOT proceed to the next seed until this one is GREEN.
If RED: report back what failed so the Engineer can fix and re-run.
```

### Step D: Loop Control

- If Validator says GREEN: proceed to the next seed.
...
- The loop is NOT complete until ALL 25 seeds are GREEN.
...
3. EVERY seed must have ≥2 hero, ≥1 broll, ≥1 graphic. Missing = FAIL.
...
6. Commit code fixes immediately: git add -A && git commit -m "fix: <desc> (Seed #N)"
7. The loop continues until ALL 25 seeds are GREEN.

## Seeds (from SEEDS.md)

Process in order. The seed list is at docs/plans/e2e_smoke_loop/SEEDS.md.

Start with Seed #1 now.
