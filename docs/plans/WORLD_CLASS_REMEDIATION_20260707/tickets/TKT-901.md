...
Any step requiring a REAL paid API call (Higgsfield Seedance/ElevenLabs, vision LLM at volume, stock API free-tier-exceeded, flagship budget tier) is BLOCKED until the user explicitly authorizes in the session prompt. Each such step reports:
```
...
- G1: All WCR rules operational on one production (via inspect report).
